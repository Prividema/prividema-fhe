                ...
                for(int64_t p = 0 ; p < NBASE ; p++)
                {
                    ...
                    for(int64_t i = LBASE ; i >= 1 ; i--)
                    {
                        ...
                        cr_assert(eq(i64, (res_mat_ii_jj[(i-1)*(KBASE+1)*NBASE + j*NBASE + p] - (acc + remainder)) % (1 << KAPPABASE), 0),
                                "Equality failed at ii = %ld jj = %ld j = %ld p = %ld i = %ld with acc = %ld reminder = %ld and res = %ld", 
                                ii, jj, j, p, i, acc, remainder, res_mat_ii_jj[(i-1)*(KBASE+1)*NBASE + j*NBASE + p]);
...
Test(ggsw_Sj_Yti_dft, basic){
    ...
    for(int64_t i = 1 ; i < nb_partials(params) ; i++)
        for(int64_t j = 0 ; j < K_TILDEBASE + 1 ; j++)
        {
            ...
            cr_assert(eq(dbl, ct_dft->mat[(i-1)*(K_TILDEBASE + 1)*NLIMBSBASE*NBASE + j*NLIMBSBASE*NBASE], 0.1));
            cr_assert(eq(dbl, ct_dft->mat[(i-1)*(K_TILDEBASE + 1)*NLIMBSBASE*NBASE + j*NLIMBSBASE*NBASE + 1], 0.2));
        }
...
Test(add_ggsw_dft, basic)
{
    ...
    for (int64_t i = 0 ; i < nb_rows ; i++)
        for(int64_t j = 0 ; j < nb_cols ; j++)
            for (uint64_t p = 0 ; p < N ; p++)
                cr_assert(eq(res_dft->mat[i*N*nb_cols + j*N + p], ct_l_dft->mat[i*N*nb_cols + j*N + p] + ct_r_dft->mat[i*N*nb_cols + j*N + p]));
