...
PolyUniv** new_ggsw_secret_key_values(uint64_t N, uint64_t k
    ...
    if(log_is_null(values, "values's malloc failed in new_ggsw_secret_key_values.") < 0) 
        return NULL;
...
PolyUniv** new_uniform_ggsw_secret_key_values(MODULE* module, uint64_t k, uint64_t  nb_bits
){
    uint64_t N = module->nn;
    PolyUniv** values = malloc(k*sizeof(PolyUniv*));
    if(log_is_null(values, "values's malloc failed in new_uniform_ggsw_secret_key_values.") < 0)
        return NULL;
...
GGSWSecretKey* new_ggsw_secret_key(PolyUniv** values, uint64_t N, uint64_t k
    ...
    if(log_is_null(sk, "sk's malloc failed in new_ggsw_secret_key.") < 0)
        return NULL;
...
GGSWSecretKey* new_uniform_ggsw_secret_key(MODULE* module, uint64_t k, uint64_t nb_bits)
{
    uint64_t N = module->nn;

    GGSWSecretKey* sk = malloc(sizeof(GGSWSecretKey));
    if(log_is_null(sk, "sk's malloc failed in new_uniform_ggsw_secret_key.") < 0)
        return NULL;
...
GGSWSecretKey* transform_ggsw_secret_key_dft_to_not_dft(MODULE* module, GGSWSecretKeyDFT* sk_dft
){
...
PolyUniv** transform_ggsw_secret_key_values_dft_to_not_dft(MODULE* module, PolyUnivDFT** values_dft, uint64_t k
){
    uint64_t N = module->nn;
    ...
    {
        values[j] = calloc(N, sizeof(int64_t));
        if(log_is_null(values[j], "values elements' calloc failed in transform_ggsw_secret_key_values_dft_to_not_dft.") < 0)
        {
            for(int64_t t = 0 ; t < j ; t++)
                free(values[t]);
            free(values);
...
PolyUnivDFT** new_ggsw_secret_key_values_dft(uint64_t N, uint64_t k
    ...
    if(log_is_null(values_dft, "values_dft's malloc failed in new_ggsw_secret_key_values_dft.") < 0)
        return NULL;
...
PolyUnivDFT** new_uniform_ggsw_secret_key_values_dft(MODULE* module, uint64_t k, uint64_t nb_bits
){
    uint64_t N = module->nn;
    PolyUnivDFT** values_dft = malloc(k*sizeof(PolyUnivDFT*));
    ...
    for(int64_t j = 0 ; j < k ; j++)
    {
        ...
        if(log_is_null(values_dft[j], "values_dft elements' calloc failed in new_uniform_ggsw_secret_key_values_dft.") < 0)
        {
            for(int64_t t = 0 ; t < j ; t++)
                free(values_dft[t]);
            free(values_dft);
...
GGSWSecretKeyDFT* new_ggsw_secret_key_dft(PolyUnivDFT** values, uint64_t N, uint64_t k
    ...
    if(log_is_null(sk_dft, "sk_dft's malloc failed in new_ggsw_secret_key_dft.") < 0)
        return NULL;
...
GGSWSecretKeyDFT* new_uniform_ggsw_secret_key_dft(MODULE* module, uint64_t k, uint64_t nb_bits
){
    uint64_t N = module->nn;

    GGSWSecretKeyDFT* sk_dft = malloc(sizeof(GGSWSecretKeyDFT));
    if(log_is_null(sk_dft, "sk_dft's malloc failed in new_uniform_ggsw_secret_key_dft.") < 0)
        return NULL;
...
GGSWSecretKeyDFT* transform_ggsw_secret_key_not_dft_to_dft(MODULE* module, GGSWSecretKey* sk
){
...
PolyUnivDFT** transform_ggsw_secret_key_values_not_dft_to_dft(MODULE* module, PolyUniv** values, uint64_t k
){
    uint64_t N = module->nn;
    ...
    {
        values_dft[j] = calloc(N, sizeof(double));
        if(log_is_null(values_dft[j], "values_dft elements' calloc failed in transform_ggsw_secret_key_values_not_dft_to_dft.") < 0)
        {
            for(int64_t t = 0 ; t < j ; t++)
                free(values_dft[t]);
            free(values_dft);
...
GLWESecretKeyDFT* transform_ggsw_secret_key_dft_to_glwe_secret_key_dft(GGSWSecretKeyDFT* sk_ggsw_dft){
    ...
    if(log_is_null(sk_glwe_dft, "sk_glwe_dft's malloc failed in transform_ggsw_secret_key_dft_to_glwe_secret_key_dft.") < 0)
        return NULL;
