                        ...
                        PolyUniv* msg_univ)
{
    ...
    if(log_is_null(msg_univ_dft, "msg_univ_dft's malloc failed in ggsw_secret_encrypt.") < 0)
        return -1;
    ...
    {
        free(msg_univ_dft); free(m_skj_univ_dft);
        return -1;
    }
    ...
    {
        free(msg_univ_dft); free(m_skj_univ_dft); free(m_skj_univ);
        return -1;
    }
    ...
    {
        free(msg_univ_dft); free(m_skj_univ_dft); free(m_skj_univ); free(phase_univ_RnX);
        return -1;
    }
    ...
    for(int64_t i = 1 ; i <= nb_partials(params_ggsw) ; i++)
    {
        for(int64_t j = 0 ; j < k_tilde + 1 ; j++)
        {
            ...
            if (compute_phase_ij(module, params_ggsw, sk_dft, msg_univ, msg_univ_dft, m_skj_univ, m_skj_univ_dft,
                                 phase, phase_univ_RnX, i, j) < 0)
            {
                free(phase); free(msg_univ_dft); free(m_skj_univ_dft); free(m_skj_univ); free(phase_univ_RnX);
...
int ggsw_external_product(MODULE* module, 
    ...
    if(log_is_null(mat_dft, "mat_dft's malloc failed in ggsw_external_product.") < 0)
        return -1;
                                       ...
                                       VecBivDFT* ct_dft)
{
    ...
    if(log_is_null(ct, "ct's malloc failed in glwe_secret_demasking_ggsw_lib_dft.") < 0)
        return -1;
                                     ...
                                     PolyBivDFT* phase_dft)
{
    ...
    if(log_is_null(tmp_ct, "tmp_ct's malloc failed.") < 0)
        return -1;
                            ...
                            PolyUniv* msg_univ)
{
    ...
    // Computes DFT(msg)
    PolyUnivDFT* msg_univ_dft = malloc(poly_univ_bytes(params_glwe));
    if(log_is_null(msg_univ_dft, "msg_univ_dft's malloc failed.") < 0)
        return -1;
    vec_znx_dft_p(module, msg_univ_dft, 1, msg_univ, 1, N);
    ...
    {
        free(msg_univ_dft); free(m_skj_univ_dft); free(m_skj_univ); free(phase_univ_RnX); free(phase);
        return -1;
    }
    ...
    for(int64_t i = 1 ; i <= nb_partials(params_ggsw) ; i++)
    {
        for(int64_t j = 0 ; j < k_tilde + 1 ; j++)
        {
            ...
            {
                free(phase); free(m_skj_univ); free(phase_univ_RnX);
                free(phase_dft); free(msg_univ_dft); free(m_skj_univ_dft); 
    ...
    free(phase); free(m_skj_univ); free(phase_univ_RnX);
    free(phase_dft); free(m_skj_univ_dft); free(msg_univ_dft);  
