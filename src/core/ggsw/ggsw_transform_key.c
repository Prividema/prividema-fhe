...
void transform_ggsw_secret_key_not_dft_to_dft(const MODULE* module, GGSWSecretKeyDFT* result, const GGSWSecretKey* sk)
{
	for (uint64_t j = 0; j < sk->k; j++) 
		vec_znx_dft_p(module, result->values[j], 1, sk->values[j], 1, sk->N);
}

void transform_ggsw_secret_key_dft_to_glwe_secret_key_dft(GLWESecretKeyDFT* result, const GGSWSecretKeyDFT* sk_ggsw_dft)
{
	for (uint64_t j = 0; j < sk_ggsw_dft->k; j++)
		for (int p = 0; p < sk_ggsw_dft->N; p++) 
			result->values[j][p] = sk_ggsw_dft->values[j][p];
}
