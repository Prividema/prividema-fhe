...
int const_mult_glwe(const MODULE* module, GLWECiphertext* result, const PolyUnivDFT* u_dft, const GLWECiphertext* glwe, int do_normalization)
{
	int status = -1;
...
void add_glwe_dft(GLWECiphertextDFT* result_dft, const GLWECiphertextDFT* glwe_lhs_dft, const GLWECiphertextDFT* glwe_rhs_dft)
{
    for (uint64_t t = 0; t < glwe_coef_number(result_dft->params); t++)
        result_dft->vec[t] = glwe_lhs_dft->vec[t] + glwe_rhs_dft->vec[t];
}

int const_mult_glwe_dft(const MODULE* module, GLWECiphertextDFT* result_dft, const PolyUnivDFT* u_dft, const GLWECiphertextDFT* glwe_dft,
...
void mult_vec_znx_dft(const MODULE* module, double* result_dft, int64_t result_size, const double* c_dft, int64_t c_size,
