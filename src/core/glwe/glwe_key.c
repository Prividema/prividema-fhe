...
PolyUniv** new_glwe_secret_key_values(uint64_t N, uint64_t k
    ...
    if(log_is_null(values, "values' malloc failed in new_glwe_secret_key_values.") < 0)
        return NULL;
...
PolyUniv** new_uniform_glwe_secret_key_values(MODULE* module, uint64_t k, uint64_t  nb_bits)
{
    ...
    PolyUniv** values = malloc(k*sizeof(PolyUniv*));
    if(log_is_null(values, "values' malloc failed in new_uniform_glwe_secret_key_values.") < 0)
        return NULL;
...
GLWESecretKey* new_glwe_secret_key(PolyUniv** values, uint64_t N, uint64_t k
    ...
    if(log_is_null(sk, "sk's malloc failed in new_glwe_secret_key.") < 0)
        return NULL;
...
GLWESecretKey* new_uniform_glwe_secret_key(MODULE* module, uint64_t k, uint64_t nb_bits)
{
    ...
    GLWESecretKey* sk = malloc(sizeof(GLWESecretKey));
    if(log_is_null(sk, "sk's malloc failed in new_uniform_glwe_secret_key.") < 0)
        return NULL;
...
PolyUniv** transform_glwe_secret_key_values_dft_to_not_dft(MODULE* module, PolyUnivDFT** values_dft, uint64_t k
    ...
    {
        values[j] = calloc(N, sizeof(int64_t));
        if(log_is_null(values[j], "values elements' malloc failed in transform_glwe_secret_key_values_dft_to_not_dft.") < 0)
        {
            for(int64_t t = 0 ; t < j ; t++)
                free(values[t]);
            free(values);
...
PolyUnivDFT** transform_glwe_secret_key_values_not_dft_to_dft(MODULE* module, PolyUniv** values, uint64_t k
    ...
    for(int64_t j = 0 ; j < k ; j++)
    {
        ...
        if(log_is_null(values_dft[j], "values_dft elements' calloc failed in transform_glwe_secret_key_values_not_dft_to_dft") < 0)
        {
            for(int64_t t = 0 ; t < j ; t++)
                free(values_dft[t]);
            free(values_dft);
