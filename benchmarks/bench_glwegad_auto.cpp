...
void bench_glwegad_auto(benchmark::State& state)
{
...
BENCHMARK(bench_glwegad_auto);

void bench_glwegad_auto_norm(benchmark::State& state)
{
	PvdaBackend* module = pvda_new_spqlios_backend(NBASE);
	GLWEParams* params_glwe =
	    new_glwe_params(NBASE, KBASE, KAPPABASE, NLIMBSBASE, SIGMABASE, NOISE_UNIFORM_POWER_OF_TWO);
	...
	{
		glwegadget_automorphism(module, glwe_res, auto_key, glwe_ct);
		normalize_glwe(module, glwe_ct, glwe_ct);
		benchmark::DoNotOptimize(glwe_res);
	}
...
BENCHMARK(bench_glwegad_auto_norm);
